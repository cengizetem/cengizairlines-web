...
require '../connect.php';
  ...
  $jaratdata = $connection->query("SELECT jarat.jaratSzam, jarat.repuloRegisztracioKod, jarat.indulas, jarat.erkezes, honnan.varos as honnan, hova.varos as hova
  FROM jaratok jarat
  INNER JOIN varosok honnan
      on jarat.honnan_varosId = honnan.id
  INNER JOIN varosok hova
      on jarat.hova_varosId = hova.id
    WHERE jarat.jaratSzam NOT in (SELECT jarat.jaratSzam
  FROM jaratok jarat
  INNER JOIN varosok honnan
      on jarat.honnan_varosId = honnan.id
  INNER JOIN varosok hova
